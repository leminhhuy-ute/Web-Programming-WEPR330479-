...
const LecturerCrud = {
    ...
    renderRecentTopics(topics) {
        ...
        tbody.innerHTML = topics.map(t => `
            <tr>
                ...
                <td><a href="/lecturer/topics/detail.html?id=${t.id}">${escapeHtml(t.title)}</a></td>
    ...
    renderTopicTable(topics) {
        ...
        tbody.innerHTML = topics.map(t => `
            <tr>
                ...
                <td>${escapeHtml(t.createdBy ? t.createdBy.fullName : '—')}</td>
                <td><span class="badge ${t.topicType.toLowerCase()}">${escapeHtml(lecturerLabel(t.topicType))}</span></td>
                <td>${escapeHtml(t.department ? t.department.code : '—')}</td>
                <td><span class="badge ${t.status.toLowerCase()}">${escapeHtml(lecturerLabel(t.status))}</span></td>
                <td>
                    <a href="detail.html?id=${t.id}" class="button outline">Chi tiết</a>
                    ${t.createdBy?.id === this.currentUser?.id && t.status !== 'APPROVED' ? `<a href="form.html?id=${t.id}" class="button primary">Sửa</a>` : ''}
                    ${t.createdBy?.id === this.currentUser?.id && t.status !== 'APPROVED' ? `<button onclick="LecturerCrud.deleteTopic(${t.id})" class="button danger">Xóa</button>` : ''}
                </td>
            </tr>
        `).join('');
    },
    ...
    async renderApprovalTable(topics) {
        ...
        tbody.innerHTML = topics.map(t => {
            const adv1Options = lecturers.map(l => `<option value="${l.id}" ${t.advisor1 && t.advisor1.id === l.id ? 'selected' : ''}>${escapeHtml(l.fullName)}</option>`).join('');
            const adv2Options = `<option value="">-- Không gán --</option>` + lecturers.map(l => `<option value="${l.id}" ${t.advisor2 && t.advisor2.id === l.id ? 'selected' : ''}>${escapeHtml(l.fullName)}</option>`).join('');
            ...
            <tr>
                ...
                <td>
                    <div style="display:flex; flex-direction:column; gap:4px;">
                        ...
                        <button ${t.status !== 'APPROVED' ? 'disabled' : ''} onclick="LecturerCrud.assignAdvisors(${t.id})" class="button outline" style="padding:2px 6px; font-size:11px;">Gán GVHD</button>
                    </div>
                </td>
                <td>
                    ${t.status === 'PENDING' ? `
        ...
        try {
            ...
            tbody.innerHTML = groups.map(g => `
                <tr>
                    ...
                    <td><a href="detail.html?id=${g.topic.id}">${escapeHtml(g.topic.title)}</a> (${escapeHtml(g.topic.topicCode)})</td>
                    <td><strong>${escapeHtml(g.leader ? g.leader.fullName : '—')}</strong> (${escapeHtml(g.leader ? g.leader.userCode : '')})</td>
                    ...
                    <td>
                        ...
                        ` : `
                            <span style="font-size:12px; color:#666;">${escapeHtml(g.notes || 'Đã xử lý')}</span>
                        `}
                    </td>
                </tr>
            `).join('');
